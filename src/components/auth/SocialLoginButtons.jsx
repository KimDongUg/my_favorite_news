/**
 * 소셜 로그인 버튼 컴포넌트
 */

import { useState, useEffect } from 'react';
import { useAuth } from '../../contexts/AuthContext';
import { AUTH_BASE_URL } from '../../config/api';

const socialProviders = [
  {
    id: 'google',
    name: 'Google',
    icon: (
      <svg viewBox="0 0 24 24" width="20" height="20">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
      </svg>
    ),
    color: '#fff',
    textColor: '#333',
    border: '1px solid #ddd',
  },
  {
    id: 'kakao',
    name: '카카오',
    icon: (
      <svg viewBox="0 0 24 24" width="20" height="20">
        <path fill="#000" d="M12 3c5.799 0 10.5 3.664 10.5 8.185 0 4.52-4.701 8.184-10.5 8.184a13.5 13.5 0 0 1-1.727-.11l-4.408 2.883c-.501.265-.678.236-.472-.413l.892-3.678c-2.88-1.46-4.785-3.99-4.785-6.866C1.5 6.665 6.201 3 12 3z"/>
      </svg>
    ),
    color: '#FEE500',
    textColor: '#000',
    border: 'none',
  },
  {
    id: 'naver',
    name: '네이버',
    icon: (
      <svg viewBox="0 0 24 24" width="20" height="20">
        <path fill="#fff" d="M16.273 12.845L7.376 0H0v24h7.727V11.155L16.624 24H24V0h-7.727v12.845z"/>
      </svg>
    ),
    color: '#03C75A',
    textColor: '#fff',
    border: 'none',
  },
];

export default function SocialLoginButtons() {
  const [availableProviders, setAvailableProviders] = useState([]);
  const [loading, setLoading] = useState(true);
  const { getAvailableProviders } = useAuth();

  useEffect(() => {
    const fetchProviders = async () => {
      const providers = await getAvailableProviders();
      setAvailableProviders(providers.map(p => p.id));
      setLoading(false);
    };
    fetchProviders();
  }, [getAvailableProviders]);

  const handleSocialLogin = (providerId) => {
    // OAuth 로그인 시작 - 백엔드로 리다이렉트
    window.location.href = `${AUTH_BASE_URL}/auth/${providerId}`;
  };

  if (loading) {
    return (
      <div className="social-login-buttons loading">
        <div className="skeleton-btn"></div>
        <div className="skeleton-btn"></div>
        <div className="skeleton-btn"></div>
      </div>
    );
  }

  // 사용 가능한 프로바이더가 없으면 모든 프로바이더를 비활성화 상태로 표시
  const displayProviders = availableProviders.length > 0
    ? socialProviders.filter(p => availableProviders.includes(p.id))
    : socialProviders;

  const isDisabled = availableProviders.length === 0;

  return (
    <div className="social-login-buttons">
      {displayProviders.map((provider) => (
        <button
          key={provider.id}
          className={`social-login-btn ${provider.id} ${isDisabled ? 'disabled' : ''}`}
          style={{
            backgroundColor: provider.color,
            color: provider.textColor,
            border: provider.border,
          }}
          onClick={() => handleSocialLogin(provider.id)}
          disabled={isDisabled}
          title={isDisabled ? 'OAuth 설정이 필요합니다' : `${provider.name}로 계속하기`}
        >
          <span className="provider-icon">{provider.icon}</span>
          <span className="provider-text">{provider.name}로 계속하기</span>
        </button>
      ))}

      {isDisabled && (
        <p className="oauth-notice">
          소셜 로그인을 사용하려면 서버에 OAuth 설정이 필요합니다
        </p>
      )}
    </div>
  );
}
